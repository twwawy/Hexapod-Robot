...
from hexapod_mjx.model import LEG_NAMES, STAND_POSE, load_hexapod_model, repo_root_from
...
def _foot_body_z(bundle, data: mujoco.MjData) -> list[float]:
...
def _build_pose_data(bundle, pose: dict[str, float]) -> tuple[mujoco.MjData, float]:
    ...
    reset_root_height = -float(np.mean(np.asarray(_foot_body_z(bundle, data), dtype=np.float64)))
    data.qpos[2] = reset_root_height
    mujoco.mj_forward(bundle.model, data)
    return data, reset_root_height


def _payload(repo_root: Path, pose: dict[str, float], reset_root_height: float, args: argparse.Namespace) -> dict[str, object]:
    return {
        "repo_root": str(repo_root),
        "reset_root_height": reset_root_height,
        "stand_pose": pose,
        "camera": {
...
def main() -> None:
    ...
    data, reset_root_height = _build_pose_data(bundle, pose)
    payload = _payload(repo_root, pose, reset_root_height, args)
