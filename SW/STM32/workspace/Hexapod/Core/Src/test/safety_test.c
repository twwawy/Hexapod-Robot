...
bool SafetyTest_Run(void)
{
    ...
    ik_valid[2] = false;                        // 첫 3번 다리의 IK 실패를 넣는다.
    output = Safety_Evaluate(&handle, &imu, ik_valid);  // 첫 실패를 누적한다.
    ik_valid[2] = true;                         // IK를 한 번 정상으로 되돌린다.
    output = Safety_Evaluate(&handle, &imu, ik_valid);  // 연속 실패 초기화를 확인한다.
    if (output.controller_fault || (handle.ik_failure_count != 0U))
    {
        return false;
    }

    ik_valid[2] = false;  // 연속 IK 실패 시험을 시작한다.
    output = Safety_Evaluate(&handle, &imu, ik_valid);  // 첫 번째 연속 실패를 누적한다.
    if (output.controller_fault || (handle.ik_failure_count != 1U))
    {
        return false;
    }
    output = Safety_Evaluate(&handle, &imu, ik_valid);  // 두 번째 연속 실패를 누적한다.
    if (output.controller_fault || (handle.ik_failure_count != 2U))
    {
        return false;
    }
    output = Safety_Evaluate(&handle, &imu, ik_valid);  // 세 번째 연속 실패를 확정한다.
    ik_valid[2] = true;                                 // IK를 정상으로 되돌린다.
    if (!output.controller_fault ||
        !Safety_Evaluate(&handle, &imu, ik_valid).controller_fault)
    {
        return false;  // 세 번째 Fault와 이후 영구 Latch를 확인한다.
    }
