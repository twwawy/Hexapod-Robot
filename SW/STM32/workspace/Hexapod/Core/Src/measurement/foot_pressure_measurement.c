                                   ...
                                   FootPressure_Calibration_t table[ROBOT_PRESSURE_COUNT])
{
    ...
    for (leg = 0U; leg < ROBOT_PRESSURE_COUNT; ++leg)
    {
        ...
        if (difference > 0)
        {
            table[leg].release_threshold = (uint16_t)((int32_t)unloaded + difference / 10);     // 10% 해제 임계값을 만든다.
            table[leg].contact_threshold = (uint16_t)((int32_t)unloaded + difference * 2 / 5);  // 40% 접촉 임계값을 만든다.
        }
        else
        {
            table[leg].contact_threshold = (uint16_t)((int32_t)unloaded + difference * 2 / 5);  // 반전 40% 접촉값을 만든다.
            table[leg].release_threshold = (uint16_t)((int32_t)unloaded + difference / 10);     // 반전 10% 해제값을 만든다.
        }
