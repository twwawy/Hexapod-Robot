...
ANCHORS: dict[str, tuple[str, ...]] = {
    "mjx/firmware_mjx_controller.py": (
        ...
        r"RESIDUAL_SCALE\s*=\s*jp\.array\(\(0\.04,\s*0\.02,\s*0\.02\)\)",
        ...
        r"posture_error\s*=\s*-attitude_rpy\[:2\]",
        r"jp\.deg2rad\(15\.0\)",
        r"jp\.deg2rad\(45\.0\)",
    ...
    "mjx/rough_terrain_env.py": (
        r"OBSERVATION_SIZE\s*=\s*142",
        r"OBSERVATION_CONTRACT_VERSION\s*=\s*\"firmware_state_collision_terrain_curriculum_v2\"",
        ...
        r"& \(jp\.max\(jp\.abs\(attitude\[:2\]\)\) < jp\.deg2rad\(20\.0\)\)",
        r"torque_limit\s*=\s*SERVO_STALL_TORQUE_NM",
        r"torque_saturation\s*=",
        r"reward_terms\s*=\s*\{",
        r"\"upright\": jp\.exp\(-jp\.sum\(jp\.square\(attitude\[:2\]\)\) / 0\.12\)",
        r"scaled\s*=\s*\{",
    ),
    "mjx/train_rough_terrain.py": (
        ...
        r"1000 if spec\.level == 0 else \(2500 if spec\.level <= 8 else 5000\)",
    ...
    "mjx/terrain_curriculum.py": (
        ...
        r"_stairs\(8,\s*7,\s*0\.20\)",
        r"_stairs\(9,\s*10,\s*0\.05\)",
        r"_stairs\(12,\s*10,\s*0\.20\)",
    ),
}
