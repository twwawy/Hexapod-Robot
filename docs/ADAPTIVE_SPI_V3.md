...
The app defaults to the existing 64-byte sensor/GPS transport. The 128-byte adaptive
transport remains available through an explicit `JetsonSpi_EnableV3` call after
initialization; the Jetson master must use the matching transport. GPS is not included
in the adaptive observation pages below. No automatic frame-length negotiation.
